...
PKG := github.com/cloudogu/velero-plugin-for-aws
...
# create the correct image and pushes it into the cluster registry
.PHONY: ces-deploy
ces-deploy: container push
	@./velero plugin remove $(IMAGE) || true
	@./velero plugin add --image-pull-policy Always $(IMAGE) || true